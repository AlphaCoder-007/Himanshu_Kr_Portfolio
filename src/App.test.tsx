...
vi.mock('framer-motion', () => ({
  motion: {
    div: vi.fn().mockImplementation(({ children, className, onClick, id, role }) => (
      <div className={className} onClick={onClick} id={id} role={role}>{children}</div>
    )),
    h1: vi.fn().mockImplementation(({ children, className }) => <h1 className={className}>{children}</h1>),
    h2: vi.fn().mockImplementation(({ children, className }) => <h2 className={className}>{children}</h2>),
    p: vi.fn().mockImplementation(({ children, className }) => <p className={className}>{children}</p>),
    form: vi.fn().mockImplementation(({ children, className, onSubmit, noValidate }) => (
      <form className={className} onSubmit={onSubmit} noValidate={noValidate}>{children}</form>
    )),
...
describe('Portfolio Application Test Suite', () => {
  ...
  it('validates contact form fields and shows error state, then allows successful transmission', async () => {
    ...
    fireEvent.change(messageInput, { target: { value: 'Hello, looking for a framework consult.' } });
